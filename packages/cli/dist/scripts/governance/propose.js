...
import { getPublicClient, CONTRACTS, CATEGORIES, governorAbi, erc20Abi, } from "../lib/contracts.js";
async function main() {
    const [categoryStr, title, description] = process.argv.slice(2);
    if (!categoryStr || !title) {
        console.error('Usage: npx tsx scripts/governance/propose.ts <categoryId> "Title" "Description"');
    ...
    if (fomaBalance < maxCost) {
        console.error(`Insufficient FOMA: have ${formatUnits(fomaBalance, 18)}, need up to 100 for proposal cost`);
        process.exit(1);
    }
